...
public class ContractScroll extends Item {
    ...
    public static void writeUuid(CompoundTag tag, UUID uuid) {
        UUIDUtil.CODEC.encodeStart(NbtOps.INSTANCE, uuid).ifSuccess(value -> tag.put(NBTname, value));
    }

    public static Optional<UUID> readUuid(CompoundTag tag) {
        if (tag.contains(NBTname)) {
            return UUIDUtil.CODEC.parse(NbtOps.INSTANCE, tag.get(NBTname)).result();
        }
